  ...
  return (
    <div className="min-h-screen flex bg-gray-100">
      ...
      <div className="flex flex-col w-full p-6">
        {/* Header */}
        <div className="mb-6 border-b-4 border-blue-500 pb-2">
          <h2 className="text-4xl font-extrabold text-blue-900">Professor Details</h2>
          ...
          {professors.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {professors.map((item, index) => (
                <div
                  key={index}
                  className="p-6 border rounded-xl shadow-lg bg-white flex flex-col gap-4 transform hover:scale-105 transition-transform duration-300 hover:shadow-xl"
                >
                  {/* Professor Name */}
                  ...
                  {/* Additional Info */}
                  {!item.number && (
                    <p className="text-sm text-gray-500 italic">Phone number not provided</p>
                  )}
                </div>
              ))}
            </div>
